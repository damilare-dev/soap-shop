...
export default function PINScreen({ title, subtitle, pin, onSuccess, onBack }: PINScreenProps) {
  ...
  const press = (d: string) => {
    ...
    setError(false);
    if (next.length === 4) {
      // Support both hashed PINs and legacy plain PINs during migration
      verifyPin(next, pin).then(match => {
        // Also check plain equality for any un-migrated legacy data
        const plainMatch = next === pin;
        if (match || plainMatch) {
